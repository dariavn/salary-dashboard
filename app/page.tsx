...
function HomeContent() {
  ...
  for (const pos of positions) {
    ...
    const candidates = loadAllCandidatesForPosition(pos.slug)
    candidatesByPosition[pos.slug] = candidates
    ...
    // Also add location meta for ATS-only countries (no research data files yet)
    for (const c of candidates) {
      if (!allLocationMeta[c.location]) {
        allLocationMeta[c.location] = getLocationMeta(c.location)
      }
    }
  }
